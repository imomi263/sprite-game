...
void keyEvent(int key, int event) {
	if (key == 82 && flag == 0)
	{
		...
		flag = 1;
		Paint();
		startTimer(moveid, moveInterval);
...
void Paint()
{
	...
	clearDevice();
	putImageScale(&back, 0, 0, Width, Height);
	if(flag==1)
	putImageTransparent(&tom1, tomsprite.x, tomsprite.y, tomsprite.width, tomsprite.height, ACL_Color WHITE);
	if (flag == 2) {
		putImageTransparent(&tom2, tomsprite.x, tomsprite.y, tomsprite.width, tomsprite.height, ACL_Color WHITE);
		char str2[50];// str2[50];
		setTextSize(30);
		setTextBkColor(RGB(255, 255, 255));
		paintText(Width/2, 10, "愤怒的Tom");
	}
...
void add_dog() {
	...
	dogsprite[dognum]->x = rand() % (Width - dogsprite[dognum]->width + 2);
	dogsprite[dognum]->y = rand() % (Height - dogsprite[dognum]->height + 2);
